...
if (isset($_POST['like_pdf_content'])) {
   if ($user_id != '') {
      $content_id = $_POST['content_id'];
      $content_id = filter_var($content_id, FILTER_SANITIZE_STRING);

      // Check if the user has already liked the PDF
      $select_pdf_likes = $conn->prepare("SELECT * FROM `likes` WHERE user_id = ? AND content_id = ?");
      $select_pdf_likes->execute([$user_id, $content_id]);
...
<?php if (!empty($fetch_content['pdf'])): ?>
   <h3 class="pdf-title">View PDF Document</h3>
   <iframe src="uploaded_files/<?= $fetch_content['pdf']; ?>" style="width:100%; height:500px;" frameborder="0"></iframe>
   
   <form action="" method="post" class="like-pdf-form">
      <input type="hidden" name="content_id" value="<?= $content_id; ?>">
      <?php
         // Check if the user has liked the PDF
         $verify_pdf_likes = $conn->prepare("SELECT * FROM `likes` WHERE user_id = ? AND content_id = ?");
         $verify_pdf_likes->execute([$user_id, $content_id]);

         if($verify_pdf_likes->rowCount() > 0){
      ?>
      <button type="submit" name="like_pdf_content" class="liked"><i class="fas fa-heart"></i><span>Liked</span></button>
      <?php
         } else {
      ?>
      <button type="submit" name="like_pdf_content" class="like"><i class="far fa-heart"></i><span>Like</span></button>
      <?php
         }
      ?>
   </form>
