...
<!-- liked PDFs section starts -->
<section class="liked-pdfs">
   <h1 class="heading">Liked PDFs</h1>
   <div class="box-container">
      <?php
      // Fetch liked PDFs
      $select_pdf_likes = $conn->prepare("SELECT * FROM `likes` WHERE user_id = ? AND content_type = 'pdf'");
      $select_pdf_likes->execute([$user_id]);

      if ($select_pdf_likes->rowCount() > 0) {
         while ($fetch_pdf_likes = $select_pdf_likes->fetch(PDO::FETCH_ASSOC)) {
            $select_pdf_content = $conn->prepare("SELECT * FROM `content` WHERE id = ? LIMIT 1");
            $select_pdf_content->execute([$fetch_pdf_likes['content_id']]);
            
            if ($select_pdf_content->rowCount() > 0) {
               $fetch_pdf_content = $select_pdf_content->fetch(PDO::FETCH_ASSOC);
               ?>
               <div class="box">
                  <h3 class="title"><?= $fetch_pdf_content['title']; ?></h3>
                  <iframe src="uploaded_files/<?= $fetch_pdf_content['pdf']; ?>" style="width:100%; height:300px;" frameborder="0"></iframe>
                  <form action="" method="post" class="flex-btn">
                     <input type="hidden" name="content_id" value="<?= $fetch_pdf_content['id']; ?>">
                     <a href="view_pdf.php?get_id=<?= $fetch_pdf_content['id']; ?>" class="inline-btn">View PDF</a>
                     <input type="submit" value="Remove" class="inline-delete-btn" name="remove">
                  </form>
               </div>
               <?php
            } else {
               echo '<p class="empty">PDF content was not found!</p>';
            }
         }
      } else {
         echo '<p class="empty">Nothing added to likes yet!</p>';
