...
if(isset($_POST['update'])){
   ...
   if(!empty($pdf)){
      ...
      $update_pdf->execute([$rename_pdf, $video_id]);
...
if(isset($_POST['delete_video'])){
   ...
   $delete_video = $conn->prepare("SELECT video, pdf FROM `content` WHERE id = ? LIMIT 1");
   $delete_video->execute([$delete_id]);
   $fetch_content = $delete_video->fetch(PDO::FETCH_ASSOC);
   
   // Deleting video and pdf
   unlink('../uploaded_files/'.$fetch_content['video']);
   unlink('../uploaded_files/'.$fetch_content['pdf']);
...
<section class="video-form">
   ...
   <form action="" method="post" enctype="multipart/form-data">
      ...
      <input type="hidden" name="old_pdf" value="<?= $fecth_videos['pdf']; ?>"> 
      ...
      <input type="text" name="title" maxlength="100" required placeholder="enter content title" class="box" value="<?= $fecth_videos['title']; ?>">
      ...
      <!-- New PDF Section -->
      <?php if (!empty($fecth_videos['pdf'])): ?>
         <a href="../uploaded_files/<?= $fecth_videos['pdf']; ?>" target="_blank">View current PDF</a>
      <?php endif; ?>
      <p>Update PDF</p>
